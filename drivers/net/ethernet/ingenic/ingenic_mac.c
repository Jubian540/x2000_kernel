...
static int ingenic_mac_phy_hwrst(struct platform_device *pdev, bool init)
{
	...
	lp->pwr_gpio = of_get_named_gpio_flags(np, "ingenic,pwr-gpio", 0, &flags);
	if (lp->pwr_gpio < 0)
		dev_info(dev, "no support power gpio\n");
	else {
		lp->pwr_lvl = flags & OF_GPIO_ACTIVE_LOW ? 0 : 1;
		gpio_direction_output(lp->pwr_gpio, lp->pwr_lvl);
	}
...
static int ingenic_gmac_clk_set_rate(struct clk *clk, unsigned int rate)
{
		...
		switch (phydev->speed) {
			case 1000:
				...
				if(lp->interface == RGMII)
					ingenic_gmac_clk_set_rate(lp->clk_tx, 125000000);
				break;
			case 100:
				synopGMAC_select_speed100(gmacdev);
				if(lp->interface == RGMII)
					ingenic_gmac_clk_set_rate(lp->clk_tx, 25000000);
				break;
			case 10:
				synopGMAC_select_speed10(gmacdev);
				if(lp->interface == RGMII)
					ingenic_gmac_clk_set_rate(lp->clk_tx, 2500000);
				break;
			default:
...
static int ingenic_mdiobus_read(struct mii_bus *bus, int phy_addr, int regnum)
{
	...
	{
		if((phy_addr==0) && (regnum == MII_BMCR))/* Basic mode control register */
		{
			data = (lp->interface == RMII)?(BMCR_SPEED100 | BMCR_FULLDPLX):(BMCR_SPEED1000 | BMCR_FULLDPLX);
		}
		if((phy_addr==0) && (regnum == MII_BMSR))/* Basic mode status register  */
		{
			data = (lp->interface == RMII)?(BMSR_LSTATUS | BMSR_100FULL2 | BMSR_100FULL):(BMSR_LSTATUS);
		}
		if((phy_addr==0) && (regnum == MII_ESTATUS))/* Extended Status */
		{
			data = 0;
		}
		if((phy_addr==0) && (regnum == MII_PHYSID1))//the upper half
		{
			data = 0x5555;
		}

		if((phy_addr==0) && (regnum == MII_PHYSID2))//lower half
		{
			data = 0xaaaa;
		}
