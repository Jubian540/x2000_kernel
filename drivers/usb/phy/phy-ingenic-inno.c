...
extern void dwc2_handle_wakeup_detected(struct dwc2_hsotg *hsotg);
		...
		const char *buf, size_t count)
{
	...
	dwc2_handle_wakeup_detected(hsotg);

	if (!strcmp(iphy->hsotg_sw_switch, "device")) {
		if (dwc2_is_device_mode(hsotg)){
			printk("current is Device mode\n");
			return count;
		}
