...
static int compfb_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	hw_composer_lock(compfb->comp_master);
	/*可能和unexport产生冲突，当comp_ctx被释放时，此时应该返回非法.*/
	if(!compfb->comp_ctx) {
		hw_composer_unlock(compfb->comp_master);
		return -EINVAL;
	}
...
#if 0
/* 根据需要，可以将每个layer导出为fb节点.针对每个Layer都可以按照标准的fb操作. */
...
int hw_compfb_export(struct hw_compfb_device *compfb, unsigned int width, unsigned int height, int wback)
{
	...
	ret = hw_compfb_alloc_vidmem(compfb);
	if(ret < 0) {
	...
	for(i = 0; i < CONFIG_FB_INGENIC_NR_LAYERS; i++) {
		compfb->fbs[i] = framebuffer_alloc(sizeof(struct hw_compfb_device), compfb->dev);
		if(compfb->fbs[i] == NULL) {
			dev_err(compfb->dev, "Failed to alloc framebuffer for layer: %d\n", i);
			goto err_layer_alloc;
			return -ENOMEM;
		}

		compfb->fbs[i]->par = compfb;

		fb_layer = compfb->fbs[i];
		...
		fb_layer->var.width = width;
		fb_layer->var.height = height;

		compfb->color_modes[i] = &compfb_colormodes[6];
		/*默认使用第一个colormodes.*/
		compfb_colormode_to_var(&fb_layer->var, compfb->color_modes[i]);
		fb_layer->fix = ingenicfb_fix;
		fb_layer->fix.line_length = (fb_layer->var.bits_per_pixel * fb_layer->var.xres) >> 3;
		fb_layer->fix.smem_start = compfb->vidmem_phys[i][0];
		fb_layer->fix.smem_len = compfb->vidmem_size / CONFIG_FB_INGENIC_NR_LAYERS;
		fb_layer->screen_size = fb_layer->fix.line_length * fb_layer->var.yres;
		fb_layer->screen_base = compfb->vidmem[i][0];
		...
		dev_info(compfb->dev, "fb_layer->screen_size: %d, fb_layer->screen_base: %x\n", fb_layer->screen_size, fb_layer->screen_base);
	}

	/* writeback framebuffer. */
	compfb->wbfb = framebuffer_alloc(sizeof(struct hw_compfb_device), compfb->dev);
	if(compfb->wbfb == NULL) {
		dev_err(compfb->dev, "Failed to alloc framebuffer for writeback\n");
		goto err_wback_alloc;
	}

	compfb->wbfb->par 		= compfb;
	fb_videomode_to_modelist(compfb->active_video_mode, 1, &compfb->wbfb->modelist);
	compfb_videomode_to_var(&compfb->wbfb->var, compfb->active_video_mode);
	compfb->wbfb->fbops 		= &compfb_wback_ops;
	compfb->wbfb->flags 		= FBINFO_DEFAULT;
	compfb->wbfb->var.width		= width;
	compfb->wbfb->var.height	= height;

	compfb->wb_color_mode		= &compfb_wback_colormodes[0];
	compfb_colormode_to_var(&compfb->wbfb->var, compfb->wb_color_mode);

	compfb->wbfb->fix		= ingenicfb_fix;
	compfb->wbfb->fix.line_length	= (compfb->wbfb->var.bits_per_pixel * compfb->wbfb->var.xres) > 3;

	compfb->wbfb->fix.smem_start	= compfb->wback_phys;
	compfb->wbfb->fix.smem_len	= compfb->vidmem_size;
	compfb->wbfb->screen_size	= compfb->wbfb->fix.line_length * compfb->wbfb->var.yres;
	compfb->wbfb->screen_base	= compfb->wback_vidmem;
	compfb->wbfb->pseudo_palette	= compfb->pseudo_palette;
	ret = register_framebuffer(compfb->wbfb);
	if(ret < 0) {
		dev_err(compfb->dev, "Failed to register framebuffer wback\n");
	}
	dev_info(compfb->dev, "wbfb screen_size: %d, screen_base: %x\n",
			compfb->wbfb->screen_size, compfb->wbfb->screen_base);


	if(wback) {

		hw_compfb_wback_comp_info(compfb);
		hw_compfb_default_comp_info(compfb);

		hw_composer_setup(compfb->comp_ctx, &compfb->comp_info);

	} else {

		hw_compfb_directout_comp_info(compfb);
...
int hw_compfb_export2(struct hw_compfb_device *compfb, struct export_info *info)
{
	...
	compfb->comp_master = compfb->comp_ctx->master;
