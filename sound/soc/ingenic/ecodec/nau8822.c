...
//	nau8822->pwdn_gpiod = devm_gpiod_get_optional(&i2c->dev, "pwdn",
//						     GPIOD_OUT_LOW);
//	MYDBG("nau8822->pwdn_gpiod=%p, gpio=%d", nau8822->pwdn_gpiod, nau8822->pwdn_gpiod->gpio);
//	if (IS_ERR(nau8822->pwdn_gpiod))
//		return PTR_ERR(nau8822->pwdn_gpiod);
//
//	gpiod_set_value_cansleep(nau8822->pwdn_gpiod, 0);
	gpio = of_get_named_gpio_flags(i2c->dev.of_node, "ingenic,pwdn-gpio", 0, &flags);
	MYDBG("GPIO pwdn=%d", gpio);
	if(gpio_is_valid(gpio)) {
		//info->reset.pin = gpio;
		//info->reset.active_level = (flags == OF_GPIO_ACTIVE_LOW) ? 0 : 1;
		MYDBG("GPIO pwdn=%d, output 0", gpio);		gpio_direction_output(gpio, 0);
		//MYDBG("GPIO pwdn=%d, output 1", gpio);		gpio_direction_output(gpio, 1);
		mdelay(1000);
	}
	gpio = of_get_named_gpio_flags(i2c->dev.of_node, "ingenic,spken-gpio", 0, &flags);
	MYDBG("GPIO spken=%d", gpio);
	if(gpio_is_valid(gpio)) {
		//info->reset.pin = gpio;
		//info->reset.active_level = (flags == OF_GPIO_ACTIVE_LOW) ? 0 : 1;
		//MYDBG("GPIO pwdn=%d, output 0", gpio);		gpio_direction_output(gpio, 0);
